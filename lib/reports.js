...
module.exports.create = function(app) {
    ...
    app.api.reports.newRecord = function(userId, accountId, date, step, comment) {
        var record = {
    ...
    app.api.reports.add = function(userId, accountId, date, step, comment) {
        var user = app.api.user.get(userId),
            record = app.api.reports.newRecord(userId, accountId, date, step, comment)
            ;
    ...
    app.api.reports.send = function(userId, email, records) {
        ...
        if (_.isObject(userRecords)) {
            ...
            report += "+----------------------------------------------------------------+\n";
            report += "+  #  |Developer|    Date    |Step|   Decision                   |+\n";
            report += "+----------------------------------------------------------------+\n";
            ...
            .then(function() {
                if (reportData.wrongRecords) {
                    return app.messages["accounting-send-result-fraudolent"];
                } else {
